...
class MessageBox:
    ...
    def ask_input(
        ...
        CTkLabel(win, text=message, text_font=Defont.add(12)).pack(pady=10)

        _input = ttk.Entry(win, font=Defont.add(12), show='*' if not show else '')
        _input.pack(pady=20)

        CTkButton(win, text='Ok', command=_get_input, width=100).place(relx=0.25, rely=0.8)
        CTkButton(win, text='Cancel', command=win.destroy, width=100, fg_color=win['bg']).place(relx=0.45, rely=0.8)
        win.mainloop()
