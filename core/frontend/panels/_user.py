...
from ...backend.utils import Color, Defont, ThreadPool, chunk, get_age, rand_id
...
class _AddUser:
    ...
    def _parse_entries(self, data):
        ...
        data['id'] = rand_id() # should be completely random in a list of million numbers
        data['linked_id'] = self.main.user.id
        return data
    ...
    def _submit(self):
        ...
        if empty:
            ...
            self.main.db.add_rec('users', data)
...
class _DoctorUpdates:
    ...
    def _create_frame(self, frame: CTkFrame, update: Update, r: int, c: int):
        ...
        update_frame.grid(row=r if c==0 else r-1, column=c, 
                          padx=20, pady=30, sticky='nw', ipadx=300)

    def _load_panel(self, frame: CTkFrame):
        user: User = self.main.user

        updates = self.db.get_updates(user.id)
        if not updates.is_empty:
            ...
            def _load():
                ...
                for update in updates[:6]:
                    if not update.read:
                        self._create_frame(frame, update, r, next(c))
                        r += 1
            self.main.after(1000, _load)
