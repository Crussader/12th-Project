    ...
    def run():
        ...
        for start in numpy.arange(0, 1, .25):
            ...
            for num in numpy.arange(start, start+.25, 0.00025):
                prog_bar.set(num)
                loading_text.set_text(text=f"{round(num, 2)*100}%")
                win.update()
...
class Message:
    ...
    @staticmethod
    def create(type_: str, message: str, yes_no=False, command=None):
        ...
        if yes_no:
            ...
            CTkButton(win, text="Yes", command=command, width=100).place(relx=0.45, rely=0.8)
            CTkButton(win, text="No", command=win.destroy, 
                      fg_color=models.Color.FRAME, width=100).place(relx=0.7, rely=0.8)
        win.mainloop()

    @classmethod
    def show_error(cls, message: str):
        cls.create(type_="Error", message=message)
    
    @classmethod
    def show_warning(cls, message: str):
        cls.create(type_="Warning", message=message)

    @classmethod
    def show_info(cls, message: str, yes_no=False):
...
class Login:
    ...
    def _sign_up(self,fields):
        ...
        data = {k.strip('*'): func(v) for k, v in fields}
